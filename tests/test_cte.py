...
from bemyeyes_django_cte import CTEManager, With
...
class TestCTE(TestCase):
    def test_simple_cte_query(self):
        cte = With(Order.objects.values("region_id").annotate(total=Sum("amount")))
    ...
    def test_cte_name_escape(self):
        totals = With(
            Order.objects.filter(region__parent="sun")
            .values("region_id")
    ...
    def test_cte_queryset(self):
        sub_totals = With(
            Order.objects.values(region_parent=F("region__parent_id")).annotate(
                total=Sum("amount")
            ),
        )
        regions = (
    ...
    def test_cte_queryset_with_model_result(self):
        ...
        data = sorted((x.region_id, x.amount, x.region_parent) for x in orders)[:5]
        self.assertEqual(
            data,
            [
                ("earth", 30, "sun"),
    ...
    def test_named_simple_ctes(self):
        ...
        region_count = With(
            Region.objects.filter(parent="sun")
            .values("parent")
    ...
    def test_named_ctes(self):
        ...
        data = sorted((r.name, r.orders_count, r.region_total) for r in root_regions)
        self.assertEqual(
            data,
            [
    ...
    def test_materialized_option(self):
        ...
        self.assertTrue(
            str(orders.query).startswith('WITH RECURSIVE "cte" AS MATERIALIZED')
        )

    def test_update_cte_query(self):
        cte = With(
            Order.objects.values(region_parent=F("region__parent_id"))
            .annotate(total=Sum("amount"))
    ...
    def test_outerref_in_cte_query(self):
        ...
        min_and_max = With(
            Order.objects.filter(region=OuterRef("pk"))
        ...
        regions = Region.objects.annotate(
            difference=Subquery(
                min_and_max.queryset()
                .with_cte(min_and_max)
                .annotate(
    ...
    def test_experimental_left_outer_join(self):
        totals = With(
            Order.objects.values("region_id")
            .annotate(total=Sum("amount"))
    ...
    def test_non_cte_subquery(self):
        ...
        sub_totals = With(
            Order.objects.values(region_parent=F("region__parent_id")).annotate(
                ...
                non_cte_subquery=Exists(User.objects.filter(pk=OuterRef("user_id"))),
            ),
        )
        regions = (
            Region.objects.all()
    ...
    def test_union_query_with_cte(self):
        orders = Order.objects.filter(region__parent="sun").only("region", "amount")
        ...
        earth_mars_cte = (
            earth_mars.with_cte(orders_cte)
